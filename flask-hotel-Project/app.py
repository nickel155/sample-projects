...
def customer_rooms_request():
    ...
    room_numbers_available = room_type_obj.get('room_numbers')
    customer_id = request.args.get('customer_id')
    ...
    new_from_time = datetime.strptime(expected_check_in, "%Y-%m-%d")
    new_to_time = datetime.strptime(expected_check_out, "%Y-%m-%d")
    ...
    for customer_booking in customer_bookings:
        ...
        old_from_time = datetime.strptime(old_from_time, "%Y-%m-%d")
        old_to_time = datetime.strptime(old_to_time, "%Y-%m-%d")
        if old_from_time < new_from_time:
            if old_to_time < new_from_time:
                already_booked = already_booked + int(customer_booking['room_count'])
                room_numbers_available = [room_number for room_number in room_numbers_available if
                                          room_number not in customer_booking['room_numbers']]
        else:
            if old_from_time < new_to_time:
                already_booked = already_booked + int(customer_booking['room_count'])
                room_numbers_available = [room_number for room_number in room_numbers_available if
                                          room_number not in customer_booking['room_numbers']]

        #
        # if (new_from_time <= old_from_time < new_to_time) and (old_to_time >= new_to_time):
        #     already_booked = already_booked + int(customer_booking['room_count'])
        # elif (old_from_time <= new_from_time) and (new_from_time < old_to_time <= new_to_time):
        #     already_booked = already_booked + int(customer_booking['room_count'])
        # elif (old_from_time <= new_from_time) and (old_to_time >= new_to_time):
        #     already_booked = already_booked + int(customer_booking['room_count'])
    ...
    if remaining_available >= int(room_count):
        ...
        booking_query = {'room_type': room_type_selected, 'room_count': room_count, 'booking_days': room_days_count,
                         ...
                         'expected_check_in': request.args.get('expected_check_in'),
                         'expected_check_out': request.args.get('expected_check_out'),
                         'payment_id': payment_id, 'status': 'Pending','room_numbers_available': room_numbers_available}
        CustomerBooking_col.insert_one(booking_query)
    return redirect(
